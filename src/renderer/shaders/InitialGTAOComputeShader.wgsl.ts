export const InitialGTAOComputeShaderWGSL = `
    ...
    fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
        ...
        let radiusPixels = clamp(params.radius * focalPixels / abs(pixelPositionView.z), params.minRadiusPixels, params.maxRadiusPixels);

        let deltaTheta = 3.14159265359 / f32(params.numSlices);

        let currentUV = (vec2<f32>(global_id.xy) + 0.5) / params.screenResolution;
        var totalVisibility: f32 = 0.0;
        
        for(var s: u32 = 0u; s < params.numSlices; s++) {
            ...
            for(var step: u32 = 1u; step <= params.maxSteps; step++) {
                let stepProgress = (f32(step) - 0.5 + (interleavedGradientNoise(vec2<f32>(f32(global_id.x), f32(global_id.y))) * 0.5)) / f32(params.maxSteps);
                ...
                let deltaDepth1 = abs(sampleDepth1 - pixelDepth);
                ...
                let deltaDepth2 = abs(sampleDepth2 - pixelDepth);
        ...
        let finalVisibility = totalVisibility / f32(params.numSlices);
